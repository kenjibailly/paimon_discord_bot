...
async function sendTeamsEmbed(event, team1Name, team2Name, team1Usernames, team2Usernames, client) {
    try {
        ...
        const formattedTeam1Usernames = team1Usernames.map(username => `- **${username}**`).join('\n') || 'No members';
        const formattedTeam2Usernames = team2Usernames.map(username => `- **${username}**`).join('\n') || 'No members';

        const embed = new EmbedBuilder()
            .setTitle(`${event.name} Teams`)
            .setDescription(`All users who have applied for **${event.name}** have now been assigned to the teams below:`)
            .addFields(
...
async function checkTeamAssignment(client) {
    try {
        ...
        if (expiredEvents.length > 0) {
            ...
            const guildIds = [...new Set(expiredEvents.map(event => event.guild_id))];

            // Fetch team assignments for all extracted guild_ids
            const teamAssignments = await TeamAssignments.find({ guild_id: { $in: guildIds } });
            ...
            const assignmentsByGuild = teamAssignments.reduce((guildMap, assignment) => {
                if (!guildMap[assignment.guild_id]) guildMap[assignment.guild_id] = [];
            ...
            for (const event of expiredEvents) {
                ...
                const team1Role = await client.guilds.cache.get(guildId).roles.fetch(team1RoleId);
                const team2Role = await client.guilds.cache.get(guildId).roles.fetch(team2RoleId);
                ...
                await sendTeamsEmbed(event, team1RoleName, team2RoleName, team1Usernames, team2Usernames, client);
