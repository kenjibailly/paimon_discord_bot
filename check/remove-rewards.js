...
const removeCustomChannel = require("./remove-reward/remove-custom-channel");
  ...
  for (const reward of old_rewards) {
    switch (reward.reward) {
      ...
      case "custom-role":
        ...
        break;
      case "custom-channel":
        await removeCustomChannel(client, reward);
        await removeReward(client, reward);
        break;
      default:
        break;
...
async function removeReward(client, reward) {
  try {
    ...
    const removed_reward = await AwardedReward.findOneAndDelete({
      _id: reward._id,
    });
    if (removed_reward) {
