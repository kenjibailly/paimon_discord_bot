...
async function loadUserSettingsIntoCache(user_id, bot_id) {
  try {
    const userSettings = await CreateImageSettings.findOne({ user_id, bot_id });
    if (userSettings) {
      cacheUserSettings(user_id, userSettings);
    }
  } catch (error) {
    console.error(
      `Failed to load user settings for user_id ${user_id}: ${error}`
    );
  }
}
