...
async function handleExchangeTrollUserButton(interaction, client) {
  try {
    ...
    try {
      const trolledUser = await trollUser(
        interaction,
        client,
        user_exchange_data
      );
    ...
    try {
      ...
      const description =
        `You have successfully trolled <@${user_exchange_data.taggedUser}>!\n` +
    ...
    if (error.code === 50013) {
      ...
      description =
        `I don't have permission to add a custom soundboard sound.\n` +
    ...
    guild.channels.cache.forEach((channel) => {
      if (channel.id !== trolledChannel.id) {
        if (
          ...
          logger.warn(
            `No permission overwrites found or insufficient permissions for channel: ${channel.name}`
          );
        }
      }
    });
    ...
    if (troll_missions.length === 0) {
      ...
      troll_missions.forEach((troll_mission, index) => {
        ...
        troll_missions_list.push({
          ...
          value: troll_mission.description
            ? troll_mission.description
      ...
      const description =
        `Oh no, you have been trolled by **${nickname}**!.\n\n` +
      ...
      try {
        ...
        throw new Error(
          "Failed to save trolled someone to the database, could it be that this person is already being trolled?"
        );
      }
