...
async function handleDimensionsButton(interaction, client) {
  let create_image_settings_termporary_user_cache =
    createImageSettingsTemporaryCache.get(interaction.user.id);
  ...
  if (!create_image_settings_termporary_user_cache) {
    ...
    create_image_settings_termporary_user_cache =
      createImageSettingsTemporaryCache.get(interaction.user.id);
    if (!create_image_settings_termporary_user_cache) {
      const defaultCheckpoint = Object.values(data)
        .flatMap((model) =>
  ...
  const parentModel = Object.values(data).find((model) =>
    model.checkpoints.some(
      (checkpoint) =>
        checkpoint.file === create_image_settings_termporary_user_cache.model
    )
  );
  ...
  if (parentModel) {
    ...
    const description =
      "Something went wrong. Perhaps the model doesn't exist anymore, choose a new model in the settings menu and try again.";
    const color = "error";
  ...
  const loraName =
    parentModel.loras.find(
      (lora) => lora.file === create_image_settings_temporary_user_cache.lora
    )?.name || "";
  ...
  if (dimensions_list.length > 0) {
    ...
    createImageSettingsUsersDataCache.set(
      interaction.user.id,
