...
async function handleCustomChannel(message, client) {
    ...
    const channelExists = existingChannels.some(channel => channel.name === channelName);
...
async function handleCustomChannelCategory(message, client) {
    ...
    user_exchange_data.category = user_exchange_data.categories[Number(messageContent) - 1];
