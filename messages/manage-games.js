...
async function handleManageGames(message, client) {
  ...
  if (
    user_exchange_data.action !== "update-game" &&
    user_exchange_data.action !== "remove-game"
  ) {
    return;
  }
  ...
  if (user_exchange_data.action == "update-game") {
    ...
    const description = `Update Game Name: **${
      user_exchange_data.games[Number(messageContent) - 1].name
    ...
    user_exchange_data.game =
      user_exchange_data.games[Number(messageContent) - 1];
    ...
    const gameRemoved = await removeGame(
      user_exchange_data.games[Number(messageContent) - 1],
      client,
      message
    );
...
async function removeGame(game, client, message) {
  try {
    ...
    if (associatedNextGames.length > 0) {
      ...
      // Fetch the token emoji using getWalletConfig function (moved up)
      const tokenEmoji = await getWalletConfig(message.guildId);
      ...
      let description =
        `The staff removed a game from the list which has been chosen as an upcoming game.\n\n` +
...
async function handleUpdateGameName(message, client) {
  ...
  const title = `Update Game`;
  const description =
    `New name: **${messageContent}** for **${user_exchange_data.game.name}**.\n\n` +
