...
async function handleChooseModel(message, client) {
  const create_image_settings_user_data_cache =
    createImageSettingsUsersDataCache.get(message.author.id);

  const messageContent = message.content;
  const validationError = validateNumber(
    messageContent,
    create_image_settings_user_data_cache.models
  );
  ...
  try {
    const selected_model =
      create_image_settings_user_data_cache.models[Number(messageContent) - 1];
    const newModel = await CreateImageSettings.findOneAndUpdate(
      { user_id: message.author.id, bot_id: client.user.id },
    ...
    await loadUserSettingsIntoCache(message.author.id, client.user.id);
    return;
  } catch (error) {
